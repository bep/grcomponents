...
// Loader is a composite that constructs the iFrame with the loader.
...
func NewIFrame(modifiers ...gr.Modifier) *Loader {
	i := &IFrame{userProvidedModifers: modifiers}
	ic := gr.New(i)
	return &Loader{iFrame: ic, loader: loadbar.NewLoader()}
}

// Frame loads the given URL in the iframe.
func (i *Loader) Frame(url string) *gr.Element {
